...
public class ReservationService {
    ...
    @DistributedLock(key = "#screenTimeId + ':' + #seatId")
    public void reserveSeat(Long screenTimeId, Long seatId, String username) {

        final User user = userService.findUser(username);
        final Optional<Reservation> existReservation = reservationRepository.findReservation(screenTimeId, seatId);
        final Long screenTime = screenTimeService.findScreenTime(screenTimeId);
        final Long seat = seatService.findSeat(seatId);
...
//    @Transactional
//    public void reserveSeatV1(Long screenId, int seatNumber, Long screenTimeId, String username) {
//
//        final User user = userService.findUser(username);
...
//    }
    ...
    public void cancelReservation(Long reservationId, String username) {
        ...
        if (reservation.getUser() == user) {
            ...
            simpMessagingTemplate.convertAndSend("/topic/seats/" + reservation.getScreenTime(), reservation);
        } else {
